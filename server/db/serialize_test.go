...
func TestSerializeDonor(t *testing.T) {
	...
	expected := `{"pubkey":"1234","expires_at":"2020-11-19T19:19:19.000752097Z"}`
...
func TestUnSerializeDonor(t *testing.T) {
	donorStr := `{"pubkey":"1234","expires_at":"2020-11-19T19:19:19.000752097Z"}`
	var donor Donor
	json.Unmarshal([]byte(donorStr), &donor)
	if donor.PubKey != "1234" {
		t.Errorf("Expected address %s but got %s", "1234", donor.PubKey)
	}
